...
const AdminApprovalNew = () => {
  const navigate = useNavigate();
  const { user } = useAuth();
  const { bookings, loading, error, fetchBookings, approveBooking, rejectBooking, deleteBooking } = useBookings();
  const [processing, setProcessing] = useState(false);

  const pendingBookings = filterBookingsByStatus(bookings, 'PENDING');

  const handleApprove = async (id) => {
    if (window.confirm('Approve this booking? This is the final approval.')) {
      setProcessing(true);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Delete this booking permanently? This action cannot be undone.')) {
      setProcessing(true);
      ...
      <div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <div>
            ...
            <p style={{ color: '#666' }}>Final approval for all bookings</p>
          </div>
          <button 
        ...
        {!loading && !error && pendingBookings.length > 0 && (
          <div>
            <div style={{ 
              ...
              <strong>{pendingBookings.length}</strong> booking(s) awaiting final approval
            </div>
...
export default AdminApprovalNew;
